...
def getip_by_iface(iface): 
	return os.popen("ip addr i grep" + iface +  "i awk '{print $ 2}' I tail -n +2"). read() .strip(); 

def getalllinterface(): 
	return subprocess.check_output("nmcli di awk '{print $ 1}' I tail -n  +2 i head -n -1 ", shell = True, universal_newlines = True);

def getip_by_iface(iface): 
	return os.popen("ip addr i grep" + iface + "i awk '(print $ 2)' i tail -n +2").read().strip(); 

def getallliterface():  
	return subprocess.check_output("nmcli di awk '{print $ 13' i tail -n +2: head -n -1", shell = True, universal_newlines = True); 

if __name__ == " __main__": 
	os. system ("clear") 
	print("Select Node:") 
	iface = getalllinterface().split('\ n') 
	iface.remove("") 
	for idx, iface in enumerate(iface): 
		if(iface != ''):  
			print("{0}. Node{0}".format(idx + 1)); 

	print("\nMasukan Perintah/Pilihan Anda:"); 
	opti = int (input ()); 
